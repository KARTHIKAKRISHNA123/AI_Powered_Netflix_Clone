...
const Moviepage = () => {
  ...
  useEffect(() => {
    // Scroll to the top of the page for a good user experience
    window.scrollTo(0, 0); 
    
    // This is the most robust data-fetching logic for finding a trailer
    const fetchMovieData = async () => {
      try {
        // Fetch the main movie details first
        const movieResponse = await fetch(`https://api.themoviedb.org/3/movie/${id}?language=en-US`, options);
        const movieData = await movieResponse.json();
        setMovie(movieData);

        // 1. Fetch ALL available videos for the movie
        const videosResponse = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-US`, options);
        const videosData = await videosResponse.json();

        // 2. First, specifically look for an official "Trailer" that is hosted on YouTube. This is the highest priority.
        const officialTrailer = videosData.results.find(video => video.type === 'Trailer' && video.site === 'YouTube');
        
        // 3. If no official trailer is found, create a fallback by looking for ANY video that is on YouTube.
        const fallbackVideo = videosData.results.find(video => video.site === 'YouTube');

        // 4. Set the key based on priority: official trailer first, then the fallback video.
        if (officialTrailer) {
          setTrailerKey(officialTrailer.key);
        } else if (fallbackVideo) {
          setTrailerKey(fallbackVideo.key);
        }
        // 5. If neither is found, trailerKey remains null, and the "Watch Trailer" button will correctly not appear.
  ...
  // Handles loading and failed API calls
  ...
  return (
    <>
            ...
            <div className='bg-[#232323] rounded-lg shadow-lg p-6 flex flex-col md:flex-row gap-8'>
                <div className='flex-1 ' >
                    <ul className='text-gray-300 space-y-3 '>
                        ...
                        <li><strong>Release Date: </strong><span className='ml-2'>{movie.release_date}</span></li>
                        <li><strong>Production Companies: </strong><span className='ml-2'>{movie.production_companies && movie.production_companies.length > 0 ? movie.production_companies.map((c) => c.name).join(", "): "N/A"}</span></li>

                        <li><strong>Countries: </strong><span className='ml-2'>{movie.production_countries && movie.production_countries.length > 0 ? movie.production_countries.map((c) => c.name).join(", "): "N/A"}</span></li>

                        <li><strong>Spoken Languages: </strong><span className='ml-2'>{movie.spoken_languages && movie.spoken_languages.length > 0 ? movie.spoken_languages.map((c) => c.english_name).join(", "): "N/A"}</span></li>
                    </ul>
                </div>
      ...
      {isPlayerOpen && trailerKey && (
        <VideoPlayer trailerKey={trailerKey} closeModal={() => setIsPlayerOpen(false)} />
      )}
    </>
  );
}
